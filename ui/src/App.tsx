...
export function App() {
   ...
   async function createLoan() {
      ...
      if (!configured) return setStatus('Contract address not configured.');
   ...
   async function lookup() {
      ...
      } catch { setLoan(null); setStatus(`No loan #${loanId} exists yet — create one with “Fund a peso loan” below.`); }
   }
      ...
      <div className="min-h-screen">
         ...
         <nav className="sticky top-0 z-20 border-b border-md-neutral-400 bg-white/85 backdrop-blur">
            <div className="mx-auto flex max-w-[960px] items-center justify-between px-4 py-3">
               ...
               <div className="flex items-center gap-2">
                  <span className="hidden items-center gap-1.5 rounded-md-pill border border-md-green-600 bg-[rgba(0,134,36,0.05)] px-2.5 py-1 text-md-b4 font-semibold text-md-green-600 sm:inline-flex">
                     <span className="h-1.5 w-1.5 rounded-full bg-md-green-600" /> Arc mainnet
                  </span>
                  <button
                     onClick={connect}
                     className="rounded-md-lg bg-md-primary-1200 px-md-3 py-md-1 text-md-b2 font-semibold text-white shadow-md-card transition-all duration-150 hover:brightness-110 active:scale-[0.97]"
                  >
         ...
         <main className="mx-auto max-w-[960px] px-4 pb-24">
            ...
            <section className="pb-6 pt-8">
               ...
               <div className="mt-4 flex flex-wrap items-center gap-2">
                  ...
                  <a
                     href={explorerAddress(PESO_LOAN_ADDRESS)}
                     target="_blank"
                     rel="noreferrer"
                     className="inline-flex items-center gap-1 rounded-md-pill border border-md-neutral-500 bg-white px-2.5 py-1 text-md-b4 font-semibold text-md-neutral-1400 hover:border-md-primary-1200 hover:text-md-primary-1200"
                  >
            ...
            <div className="grid gap-6 md:grid-cols-2">
               {/* Left: request lookup / card */}
               <div className="flex flex-col gap-3">
                  <h2 className="text-md-h5 font-semibold text-md-heading">Requests</h2>
                  ...
                  <p className="text-md-b3 text-md-neutral-1000">Loans are looked up by ID (this contract has no list method). Create one on the right, then view it here.</p>
               </div>
            ...
            <footer className="mt-12 border-t border-md-neutral-400 pt-6 text-md-b3 text-md-neutral-1200">
               <p className="mb-1">
                  <a className="font-semibold text-md-primary-1200 hover:underline" href={explorerAddress(PESO_LOAN_ADDRESS)} target="_blank" rel="noreferrer">Contract</a>
                  {' '}{short(PESO_LOAN_ADDRESS)} · USDC {short(ARC_USDC_ADDRESS)} · Arc chain {arc.id}
               </p>
               <p className="text-md-b4 text-md-neutral-1000">
                  On-chain settlement is real USDC. The PHPC / Coins.ph peso leg is <strong>simulated (preview)</strong> — PHPC is not yet live on Arc. By Moodeng Credit.
               </p>
            </footer>
         </main>
      </div>
